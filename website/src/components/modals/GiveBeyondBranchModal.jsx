...
export function GiveBeyondBranchModal({ isOpen, onClose }) {
  ...
  const handleSelectBranch = (branch) => {
    onClose && onClose();
    navigate(`/beyond-tithe?branch=${encodeURIComponent(branch.id || branch.name)}`);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      ...
      <div className="relative z-10 w-full max-w-[800px] bg-white rounded-[20px] p-6 border border-white shadow-2xl flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between pb-3 border-b border-slate-100">
          <h2 className="text-xl font-bold text-ff-secondary">Select Branch for Beyond Tithe Giving</h2>
          <button
            type="button"
            onClick={onClose}
