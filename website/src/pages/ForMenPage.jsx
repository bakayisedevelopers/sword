...
/**
 * ForMenPage matching FlutterFlow:
 * flutter-website/lib/ministries/for_men/for_men_widget.dart
 */
export function ForMenPage() {
  const { toggleDrawer } = useAppState();
  const [isSignUpOpen, setIsSignUpOpen] = useState(false);

  const { data: events } = useFirestoreQuery(COLLECTIONS.EVENTS);
  const menEvents = useMemo(() => {
    return events
      ...
      .filter((e) => {
        ...
        return (
          dept.includes('men') ||
          dept.includes('dominion') ||
          ministry.includes('men') ||
          ministry.includes('dominion') ||
          title.includes('men') ||
          title.includes('dominion')
        );
      })
      .sort((a, b) => {
  ...
  useEffect(() => {
    document.title = 'Men of Dominion | Sword of the Spirit Ministries';
  }, []);
  ...
  return (
    <div className="min-h-screen bg-white text-ff-primary-text flex flex-col selection:bg-ff-primary selection:text-ff-primary-text">
      ...
      <div className="hidden lg:block w-[90%] max-w-[1440px] mx-auto mt-[30px] mb-[20px] h-[600px] rounded-[30px] border border-ff-secondary relative overflow-hidden shadow-lg">
        <img
          src="/assets/images/Men_(2).png"
          alt="Men of Dominion Banner"
      ...
      <section className="w-[90%] max-w-[1440px] mx-auto mt-12 mb-6">
        <div className="max-w-3xl">
          ...
          <p className="text-base sm:text-lg text-slate-700 mt-3 leading-relaxed">
            Raising healthy men to raise and lead healthy families to make an impact for the body of Christ.
          </p>
        </div>
      </section>

      {/* 3. ABOUT MEN OF DOMINION */}
      <section className="w-[90%] max-w-[1440px] mx-auto my-8">
        <div className="bg-white rounded-[30px] border border-ff-secondary p-6 sm:p-10 shadow-sm space-y-4">
          <h2 className="text-2xl sm:text-3xl font-bold text-ff-secondary">
            About Men of Dominion
          </h2>
          <p className="text-sm sm:text-base text-slate-700 leading-relaxed whitespace-pre-line">
            Men of Dominion meets bi-weekly  (usually on Saturdays) throughout the branches over lots of food and the Word of God.
            {'\n\n'}
            Once a year Men have a conference around mid year to bring all Sword and Spirit men and the body of Christ me to make sure we grow as men.
          </p>
          <div className="pt-4">
            <button
              type="button"
            ...
            >
              <span>Join Men of Dominion</span>
      ...
      {/* 4. MEN OF DOMINION EVENTS */}
      {menEvents.length > 0 && (
        <section className="w-[90%] max-w-[1440px] mx-auto my-8 space-y-6">
          <h2 className="text-2xl sm:text-3xl font-bold text-ff-secondary">
            Men of Dominion Events
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {menEvents.map((evt) => {
              const eventTitle = evt.title || evt.name || 'Men of Dominion Event';
              const eventImg = evt.picture || evt.image || evt.poster || '/assets/images/Men.png';
      ...
      {isSignUpOpen && (
        <SignUpModal
          ...
          defaultMinistry="Men of Dominion"
          defaultDepartment="Men's Ministry"
        />
      )}
