...
/**
 * YoungAdultsPage matching FlutterFlow:
 * flutter-website/lib/ministries/young_adults/young_adults_widget.dart
 */
export function YoungAdultsPage() {
    ...
    return events
      ...
      .filter((e) => {
        ...
        return (
          ...
          ministry.includes('young') ||
          ministry.includes('ya') ||
          title.includes('young adult') ||
          title.includes('young adults') ||
          (ministry === 'youth' && !title.includes('teen') && !dept.includes('teen'))
        );
      })
      .sort((a, b) => {
  ...
  useEffect(() => {
    document.title = 'Young Adults | Sword of the Spirit Ministries';
  }, []);
    ...
    <div className="min-h-screen bg-white text-ff-primary-text flex flex-col selection:bg-ff-primary selection:text-ff-primary-text">
      ...
      <section className="w-[90%] max-w-[1440px] mx-auto my-8 grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white rounded-[30px] border border-ff-secondary p-6 sm:p-10 shadow-sm space-y-4">
          ...
          <p className="text-sm sm:text-base text-slate-700 leading-relaxed">
            Our Young Adults meet every week virtually.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-[30px] border border-ff-secondary overflow-hidden shadow-sm flex flex-col">
            <div className="h-[260px] w-full overflow-hidden bg-slate-100">
              <img
                src="/assets/images/evangelism1.jpg"
                alt="Ages 22 - 27"
            ...
            <div className="p-6 sm:p-8 flex-1 flex flex-col justify-between space-y-4">
              <div>
                <h3 className="text-2xl font-bold text-ff-secondary mb-2">Ages 22-27</h3>
                <p className="text-sm sm:text-base text-slate-700 leading-relaxed">
      ...
      <section className="w-[90%] max-w-[1440px] mx-auto my-8">
        <div className="bg-white rounded-[30px] border border-ff-secondary p-6 sm:p-10 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            <h3 className="text-2xl sm:text-3xl font-bold text-ff-secondary">Serve in Young Adults</h3>
            <p className="text-sm sm:text-base text-slate-700 leading-relaxed whitespace-pre-line">
              Serving in Young Adults requires a genuine relationship with Christ, a love for young people, patience, and a commitment to consistent attendance.
              {'\n\n'}
              A prayer life is important in everyone who wants to serve, because God is always speaking and it requires that each volunteer cultivates an altar.
            </p>
          </div>
          <div>
            <button
            ...
            >
              <span>Volunteer in Young Adults</span>
              <ChevronRight className="w-4 h-4" />
            </button>
          </div>
        </div>
      ...
      {yaEvents.length > 0 && (
        <section className="w-[90%] max-w-[1440px] mx-auto my-8 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {yaEvents.map((evt) => {
              const eventTitle = evt.title || evt.name || 'Young Adults Event';
      ...
      {isSignUpOpen && (
        <SignUpModal
          ...
          defaultMinistry="Young Adults"
          defaultDepartment="Young Adults"
        />
      )}
