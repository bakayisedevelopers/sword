...
/**
 * CouplesPage matching FlutterFlow:
 * flutter-website/lib/ministries/couples/couples_widget.dart
 */
export function CouplesPage() {
  ...
  const couplesEvents = useMemo(() => {
    return events
      ...
      .filter((e) => {
        ...
        return (
          dept.includes('couple') ||
          dept.includes('marriage') ||
          dept.includes('family life') ||
          ministry.includes('couple') ||
          ministry.includes('marriage') ||
  ...
  useEffect(() => {
    document.title = 'Couples Ministry | Sword of the Spirit Ministries';
  }, []);
  ...
  return (
    <div className="min-h-screen bg-white text-ff-primary-text flex flex-col selection:bg-ff-primary selection:text-ff-primary-text">
      ...
      <div className="hidden lg:block w-[90%] max-w-[1440px] mx-auto mt-[30px] mb-[20px] h-[600px] rounded-[30px] border border-ff-secondary relative overflow-hidden shadow-lg">
        <img
          src="/assets/images/Couples_(2).png"
          alt="Couples Banner"
      ...
      <section className="w-[90%] max-w-[1440px] mx-auto mt-12 mb-6">
        <div className="max-w-3xl">
          ...
          <p className="text-base sm:text-lg text-slate-700 mt-3 leading-relaxed whitespace-pre-line">
            Our Couples' ministry is ran by Apostle and Pastor Zandi, and they have proof to show for it, since they've been married for over 32 years.
            {'\n\n'}
            They don't only have years of being together but they are an example of a healthy marriage, especially for th children of God.
          </p>
        </div>
      </section>

      {/* 3. ABOUT OUR COUPLES' MINISTRY */}
      <section className="w-[90%] max-w-[1440px] mx-auto my-8">
        <div className="bg-white rounded-[30px] border border-ff-secondary p-6 sm:p-10 shadow-sm space-y-4">
          <h2 className="text-2xl sm:text-3xl font-bold text-ff-secondary">
            About Our Couples' Ministry
          </h2>
          <p className="text-sm sm:text-base text-slate-700 leading-relaxed whitespace-pre-line">
            Our couples' ministry runs gathers once a month on specified dates.
            {'\n\n'}
            Couples meet once a month for fun activities, food, dancing, laugher, and the Word of God.
          </p>
          <div className="pt-4">
            <button
              type="button"
      ...
      {couplesEvents.length > 0 && (
        <section className="w-[90%] max-w-[1440px] mx-auto my-8 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {couplesEvents.map((evt) => {
              const eventTitle = evt.title || evt.name || 'Couples Event';
              const eventImg = evt.picture || evt.image || evt.poster || '/assets/images/Couples.png';
      ...
      {isSignUpOpen && (
        <SignUpModal
          ...
          defaultMinistry="Couples"
          defaultDepartment="Family Life"
        />
      )}
