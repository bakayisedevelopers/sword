...
/**
 * SinglesPage matching FlutterFlow:
 * flutter-website/lib/ministries/singles/singles_widget.dart
 */
export function SinglesPage() {
  ...
  const singlesEvents = useMemo(() => {
    return events
      ...
      .filter((e) => {
        ...
        return (
          dept.includes('single') ||
          ministry.includes('single') ||
          title.includes('single') ||
          (ministry.includes('youth') && (title.includes('single') || dept.includes('single')))
        );
      })
      .sort((a, b) => {
  ...
  useEffect(() => {
    document.title = 'Singles Ministry | Sword of the Spirit Ministries';
  }, []);
  ...
  return (
    <div className="min-h-screen bg-white text-ff-primary-text flex flex-col selection:bg-ff-primary selection:text-ff-primary-text">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-[30px] border border-ff-secondary overflow-hidden shadow-sm flex flex-col">
            ...
            <div className="p-6 sm:p-8 flex-1 flex flex-col justify-between space-y-4">
              <div>
                <h3 className="text-2xl font-bold text-ff-secondary mb-2">Ages 28+</h3>
      ...
      <section className="w-[90%] max-w-[1440px] mx-auto my-8">
        <div className="bg-white rounded-[30px] border border-ff-secondary p-6 sm:p-10 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            <h3 className="text-2xl sm:text-3xl font-bold text-ff-secondary">Serve in Singles Ministry</h3>
            <p className="text-sm sm:text-base text-slate-700 leading-relaxed whitespace-pre-line">
              Serving in Singles Ministry requires a genuine relationship with Christ, a heart for fellowship, patience, and a commitment to consistent attendance.
              {'\n\n'}
          ...
          <div>
            ...
            >
              <span>Volunteer in Singles</span>
              <ChevronRight className="w-4 h-4" />
            </button>
          </div>
        </div>
      ...
      {/* 6. SINGLES EVENTS */}
      {singlesEvents.length > 0 && (
        <section className="w-[90%] max-w-[1440px] mx-auto my-8 space-y-6">
          <h2 className="text-2xl sm:text-3xl font-bold text-ff-secondary">
            Singles Events
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {singlesEvents.map((evt) => {
              const eventTitle = evt.title || evt.name || 'Singles Event';
      ...
      {isSignUpOpen && (
        <SignUpModal
          ...
          defaultMinistry="Singles"
          defaultDepartment="Singles Ministry"
        />
      )}
